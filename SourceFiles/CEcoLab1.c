        ...
        if ( (result == 0) && (pEnum != 0) ) {
            while (pEnum->pVTbl->Next(pEnum, 1, &cd, 0) == 0) {
                ...
                if ( (result == 0) && (pIEvents != 0) ) {
                    if (pIEvents->pVTbl->Fire_OnSortSwap != 0) {
                        result = pIEvents->pVTbl->Fire_OnSortSwap(pIEvents, index1, index2);
                    }
    ...
    if (pCMe->m_pISinkCP != 0) {
        ...
        if ( (result == 0) && (pEnum != 0) ) {
            while (pEnum->pVTbl->Next(pEnum, 1, &cd, 0) == 0) {
                result = cd.pUnk->pVTbl->QueryInterface(cd.pUnk, &IID_IEcoLab1Events, (void**)&pIEvents);
                if ( (result == 0) && (pIEvents != 0) ) {
                    if (pIEvents->pVTbl->FIre_OnSortIteration != 0) {
                        result = pIEvents->pVTbl->FIre_OnSortIteration(pIEvents, iteration);
                    }
                    pIEvents->pVTbl->Release(pIEvents);
                }
                cd.pUnk->pVTbl->Release(cd.pUnk);
...
static int16_t ECOCALLMETHOD CEcoLab1_BubbleSort(
    IEcoLab1Ptr_t me,
    int32_t* arr,
    int16_t n
) {
    ...
    for (i = 0; i + 1 < n; ++i) {
        CEcoLab1_Fire_FIre_OnSortIteration(me, (uint32_t)i);
        swapped = 0;
        ...
        for (j = 0; j + 1 < n - i; ++j) {
            if (arr[j] > arr[j + 1]) {
                ...
                CEcoLab1_Fire_Fire_OnSortSwap(me, (uint32_t)j, (uint32_t)(j + 1));
                swapped = 1;
            }
        }
